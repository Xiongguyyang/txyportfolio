...
export default function SkillCard({ title, items }: Props) {
  return (
    ...
    >
      <h3 className="font-bold text-sm tracking-widest uppercase mb-4 text-indigo-500">
        {title}
      </h3>
      <ul className="space-y-2">
        {items.map((item) => (
          <li key={item} className="flex items-center gap-2 text-base text-gray-700 dark:text-gray-300">
